...
class UserProfileController extends Controller
{
    ...
    public function all() {
        ...
        foreach ($users as $user) {
            if($user->role == 0) {      
                ...
                array_push($nurses, $user);
            }else if($user->role == 3) {    
                array_push($receptionists, $user);
            }else if($user->role == 4) {    
                array_push($pharmacists, $user);
            }else if($user->role == 5) {    
                array_push($patients, $user);
            }
        }
        $res['status']    = true;
        $res['admins']    = $admins;
        $res['doctors'] = $doctors;
        $res['nurses']  = $nurses;
        $res['receptionists']  = $receptionists;
        $res['pharmacists']  = $pharmacists;
    ...
    public function show($id) { 
        $user = User::find($id);
        if($user->role == 1 || $user->role == 2 || $user->role == 3 || $user->role == 4 || $user->role == 5) { // condition statements shows specific users except admin by id
            $res['status'] = true;
    ...
    public function update(Request $request) {  // update user information
        ...
        if ($user) {
            $user->hmo_name     = $request->input('hmo_name');
            $user->save();
